...
export const ServerList = React.memo(function ServerList({
  ...
  return (
    <div className="w-[72px] obsidian-card flex flex-col items-center py-3 gap-1 overflow-y-auto no-scrollbar shrink-0 border-none">
      {/* Home / DMs Item */}
      <div className="relative flex items-center justify-center w-full group py-0.5">
        {/* Left accent pill */}
        <div 
          className={`absolute left-0 bg-white rounded-r-full transition-all origin-left duration-200 ${
            activeTab === 'home' 
              ? 'h-10 scale-y-100' 
              : 'h-0 scale-y-0 group-hover:h-5 group-hover:scale-y-100'
          }`} 
          style={{ width: '4px' }} 
        />
      ...
      <div className="w-8 h-[2px] bg-[#2d2f31] rounded-full my-1" />
      ...
      {servers.map((server) => {
        ...
        return (
          <div key={server.id} className="relative flex items-center justify-center w-full group py-0.5">
            {/* Left accent pill */}
            <div 
              className={`absolute left-0 bg-[#e3e1db] rounded-r-full transition-all origin-left duration-200 ${
                isActive 
                  ? 'h-10 scale-y-100' 
                  : (unreadCount > 0 
                    ? 'h-2 scale-y-100' 
